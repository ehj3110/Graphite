...
def generate_surface_cage_struts(surface_faces: np.ndarray) -> np.ndarray:
    """
    Build boundary cage struts from linear (3-node) or quadratic (6-node) faces.

    For GMSH 6-node triangles (type 9): nodes are
    corners (0, 1, 2) and edge mids (3, 4, 5) for edges (0-1, 1-2, 2-0).
    We create hinged boundary segments corner->mid and mid->corner.
    """
    if surface_faces.shape[0] == 0:
        return np.empty((0, 2), dtype=np.int64)

    cage_struts: list[list[int]] = []
    if surface_faces.shape[1] == 6:
        for face in surface_faces:
            c0, c1, c2, m3, m4, m5 = [int(x) for x in face]
            cage_struts.extend(
                [
                    [c0, m3], [m3, c1],
                    [c1, m4], [m4, c2],
        ...
        for face in surface_faces:
            c0, c1, c2 = [int(x) for x in face[:3]]
            cage_struts.extend([[c0, c1], [c1, c2], [c2, c0]])

    if not cage_struts:
        return np.empty((0, 2), dtype=np.int64)
    out = np.asarray(cage_struts, dtype=np.int64)
...
def generate_topology(
    ...
    target_element_size: float | None = None,
    merge_short_struts: bool = False,
    short_strut_merge_ratio: float = 0.05,
) -> tuple[np.ndarray, np.ndarray]:
    """
    ...
    merge_short_struts:
        If True, merge nodes connected by struts shorter than
        ``short_strut_merge_ratio * target_element_size`` (union-find).
        **Warning:** a large ratio (e.g. 0.2) on dense lattices can chain-merge
        most of the graph and produce a "starburst" hub artifact. Prefer False
    ...
    if elements_np.ndim != 2 or elements_np.shape[1] < 4:
        raise ValueError(
            f"`elements` must have shape (M, >=4); got {elements_np.shape}."
        )
    if surface_faces_np.ndim != 2 or surface_faces_np.shape[1] not in (3, 6):
        raise ValueError(
    ...
    tet_xyz = nodes_np[tets_linear]  # (M, 4, 3)

    # Unique global edges and midpoints
    tet_edges = np.sort(tets_linear[:, _EDGE_PAIRS], axis=2)  # (M, 6, 2)
    ...
    tet_faces = np.sort(tets_linear[:, _FACE_TRIPLETS], axis=2)  # (M, 4, 3)
    ...
    # Strut merger (optional): dissolve very short struts to fuse U-bend junctions
    if (
        merge_short_struts
        and target_element_size is not None
        and target_element_size > 0
    ):
        unique_nodes, remap_struts = _merge_short_struts(
            unique_nodes,
            remap_struts,
            target_element_size,
            min_length_ratio=short_strut_merge_ratio,
        )

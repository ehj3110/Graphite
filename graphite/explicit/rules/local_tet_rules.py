"""
Local tetrahedral micro-rules.

Each rule takes one tetrahedron as coordinates (4, 3) and returns:
- nodes: local node coordinates produced by the rule
- struts: local edge pairs indexing into nodes

These functions are useful for documentation parity, debugging, and unit tests.
The production bulk pipeline in topology_module.py uses vectorized global forms
for performance and deterministic shared-node indexing.
"""
...
def apply_voronoi_rule(tet_coords: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    """Voronoi local rule: centroid + face-centers, hub-to-face struts."""
...
def apply_kagome_rule(tet_coords: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    """Kagome local rule: complete graph on the four face-centers."""
    coords = np.asarray(tet_coords, dtype=np.float64)
    if coords.shape != (4, 3):
        raise ValueError("tet_coords must have shape (4, 3).")
...
def apply_icosahedral_rule(tet_coords: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    """Icosahedral local rule: edge-midpoint triangles on each tet face."""
    coords = np.asarray(tet_coords, dtype=np.float64)
    if coords.shape != (4, 3):
        raise ValueError("tet_coords must have shape (4, 3).")
...
def apply_rhombic_rule(tet_coords: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    """Rhombic local rule: centroid to original tet vertices."""
    coords = np.asarray(tet_coords, dtype=np.float64)
    if coords.shape != (4, 3):
        raise ValueError("tet_coords must have shape (4, 3).")

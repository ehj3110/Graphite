...
K_MAX = 0.4
def _get_topology_overlap_factor(topology_name: str, target_vf: float = 0.10) -> float:
    # Try tet topology registry
    try:
        from graphite.explicit.rules.tet_topology_rules import get_topology_rule, normalize_topology_name
        norm_name = normalize_topology_name(topology_name)
        rule = get_topology_rule(norm_name)
        return rule.get_overlap_factor(target_vf)
    except Exception:
        pass

    # Try hex topology registry
    try:
        from graphite.explicit.hex_topology_module import get_hex_topology_rule
        rule = get_hex_topology_rule(topology_name)
        return rule.get_overlap_factor(target_vf)
    except Exception:
        pass
...
def calculate_k_analytical(
    ...
    if overlap_factor is not None:
        ...
        factor = _get_topology_overlap_factor(topology_type, target_vf)
...
def optimize_lattice_fraction(
    ...
    # Determine grid type (SC for hex, A15 for tet)
    try:
        from graphite.explicit.hex_topology_module import get_hex_topology_rule
        # If it succeeds or is registered, it's hex
        get_hex_topology_rule(topology_type)
        grid_type = "SC"
    except Exception:
        grid_type = "A15"
    ...
    if not clipped_boundary and topology_type in ("voronoi", "kagome"):
        ...
        res_0 = generate_conformal_lattice(
            cad_filepath=mesh,
            cell_size=float(target_element_size),
            strut_radius=0.1,
            lattice_type=grid_type,
            skip_sweep=True,
        )
        nodes_0 = res_0["nodes_relaxed"]
        struts_0 = np.vstack([res_0["red_struts"], res_0["cyan_struts"]]) if len(res_0["cyan_struts"]) > 0 else res_0["red_struts"]
    ...
    # Second pass: conformed background scaffold
    res = generate_conformal_lattice(
        cad_filepath=boundary_for_scaffold,
        cell_size=float(target_element_size),
        strut_radius=0.1,
        lattice_type=grid_type,
        skip_sweep=True,
    )
    nodes = res["nodes_relaxed"]
    if use_smart_inset:
        nodes = _inverse_scale_nodes_to_original(nodes, mesh, r_guess)
    struts = np.vstack([res["red_struts"], res["cyan_struts"]]) if len(res["cyan_struts"]) > 0 else res["red_struts"]

"""
Graphite Repair - Blender BMesh Repair

This script is designed to be executed headlessly via Blender's Python API 
(`blender -b -P blender_repair.py -- target.stl`). It uses BMesh and the 
built-in 3D-Print Toolbox to automatically repair non-manifold geometry, 
self-intersections, and double vertices in STL files.
"""
import bpy
import os
import sys
...
def repair_stl_with_bmesh(file_path):
    """
    Load an STL into Blender, repair it, and export a cleaned version.

    This function performs the following steps:
    1. Cleans the scene.
    2. Imports the target STL.
    3. Triangulates faces and removes duplicate vertices.
    4. Attempts to use the 3D-Print Toolbox to clean non-manifold geometry and intersections.
       Falls back to BMesh intersect if the toolbox is unavailable.
    5. Re-welds vertices and recalculates outside normals.
    6. Exports the repaired mesh with a `_ToolboxRepaired.stl` suffix.

    Parameters
    ----------
    file_path : str
        The path to the STL file to repair.
    """
    print(f"\n========================================")
